...
db.query(`SELECT article_id, body FROM comments WHERE votes < 0`).then(
  ({ rows }) => {
    console.log(rows);
  }
).catch((err) => {
...
db.query('SELECT topic FROM articles').then(({rows}) => {
  console.log(rows);
}).catch((err) => {
    return err;
  });
...
db.query(`SELECT articles.body FROM articles WHERE author = 'grumpy19'`).then(({rows}) => {
  console.log(rows);
}).catch((err) => {
    return err;
  });
...
db.query(`SELECT body FROM comments WHERE votes > 10`).then(({rows}) => {
  console.log(rows);
}).catch((err) => {
    return err;
  });
